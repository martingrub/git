...
docker-compose exec apache /usr/sbin/a2ensite  bell
docker-compose exec apache /usr/sbin/a2ensite  bell 
docker-compose exec apache /usr/sbin/a2ensite  souma

docker-compose exec apache2 /usr/sbin/a2ensite  levi
docker-compose exec apache2 /usr/sbin/a2ensite  sora
docker-compose exec apache2 /usr/sbin/a2ensite  yato
#- refrescar i apuntar 
docker-compose exec apache /usr/sbin/apache2ctl restart
docker-compose exec apache2 /usr/sbin/apache2ctl restart
...
# accedint per nom
NOM_SERVER=www.bell.com
echo -e "\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER

# accedint per nom
NOM_SERVER=www.souma.com
echo -e "\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER

# accedint per nom
NOM_SERVER=www.levi.com
echo -e "\n\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER

# accedint per nom
NOM_SERVER=www.sora.com
echo -e "\n\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER

# accedint per nom
NOM_SERVER=www.yato.com
echo -e "\n\n${yellow}Validació: comprovació des del client (per nom - $NOM_SERVER ) ${endColor}\n"
docker-compose exec textclient curl  $PROXY http://$NOM_SERVER
